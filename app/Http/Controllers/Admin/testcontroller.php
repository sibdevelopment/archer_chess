...
class testController extends Controller
{
    ...
    public function getCounts(Request $request, $coachId)
    {
        ...
        $delayedBatchesCount = \App\Models\DelayedBatch::where('coach_id', $coachId)
            ->whereBetween('date', [$startDate, $endDate])
            ->count();

        // Return the view with the calculated data
        return view('Admin.CoachReports.getcount', compact(
            'completedDemosCount',
    ...
    public function batchStudentCountryData(Request $request)
    {
        ...
        //dd($formattedStudentAttendanceData);
    ...
    public function batchCompletedData(Request $request)
    {
        ...
        $batchData = $batches->map(function ($batch) use ($completedBatchData) {
            ...
            $timeline = '';
            if ($studentBatch) {
                $startDate = Carbon::parse($studentBatch->start_date)->format('j, M Y');
                $endDate = Carbon::parse($studentBatch->end_date)->format('j, M Y');
            ...
            return [
                ...
                'timeline' => $timeline,
                'completed_count' => $completedBatchData[$batch->id]->count ?? 0, // Include the count of completed batches
    ...
    public function getSchedule(Request $request, $coachId)
    {
        ...
        if ($status === 'ACTIVE') {
            $batches = Batch::with(['batchSchedules' => function ($query) use ($dayName) {
                ...
                ->where('coach_id', $coachId)
                ->where('status', 'ACTIVE') // Ensure batches have 'ACTIVE' status
                ->get();
        } elseif ($status === 'INACTIVE') {
            $batches = Batch::with(['batchSchedules' => function ($query) use ($dayName) {
                $query->where('weekday', $dayName)
                    ->where('status', 'ACTIVE'); // Ensure batchSchedules have 'INACTIVE' status
            }])
                ->withCount(['studentBatches as inactive_students_count' => function ($query) use ($date) {
                    $query->where('status', 'INACTIVE') // Count only 'INACTIVE' studentBatches
                        ->where('start_date', '<=', $date)
                        ->where('end_date', '>=', $date);
                }])
                ->whereHas('studentBatches', function ($query) use ($date) {
                    $query->where('status', 'INACTIVE') // Ensure studentBatches have 'INACTIVE' status
                        ->where('start_date', '<=', $date)
                        ->where('end_date', '>=', $date);
                })
                ->where('coach_id', $coachId)
                ->where('status', 'INACTIVE') // Ensure batches have 'INACTIVE' status
                ->get();
        } elseif ($status === 'STANDBY') {
            $batches = Batch::with(['batchSchedules' => function ($query) use ($dayName) {
                $query->where('weekday', $dayName)
                    ->where('status', 'ACTIVE'); // Ensure batchSchedules have 'STANDBY' status
            }])
                ->withCount(['studentBatches as standby_students_count' => function ($query) use ($date) {
                    $query->where('status', 'ACTIVE') // Count only 'STANDBY' studentBatches
                        ->where('start_date', '<=', $date)
                        ->where('end_date', '>=', $date);
                }])
                ->whereHas('studentBatches', function ($query) use ($date) {
                    $query->where('status', 'ACTIVE') // Ensure studentBatches have 'STANDBY' status
                        ->where('start_date', '<=', $date)
                        ->where('end_date', '>=', $date);
                })
                ->where('coach_id', $coachId)
                ->where('status', 'STANDBY') // Ensure batches have 'STANDBY' status
                ->get();
        }
        $combinedData = [];
        foreach ($batches as $batch) {
        ...
        //dd($combinedData);
        return view('Admin.CoachReports.schedule', ['schedules' => $combinedData, 'date' => $date]);
    }
    ...
    public function getCalendarData(Request $request, $coachId)
    {
        ...
        $batchCalendarData = $this->getBatchCalendarData($coachId, $role, $todayDate, $status);
    ...
    private function getBatchCalendarData($coachId, $role, $todayDate, $status)
    {
        // Fetch batches based on the provided status
        $batches = Batch::with(['batchSchedules', 'studentBatches'])
            ->where('coach_id', $coachId)
            ->where('status', $status)
            ->get();
        ...
        foreach ($batches as $batch) {
            foreach ($batch->batchSchedules as $schedule) {
                ...
                while ($date <= $endDate) {
                    ...
                    if (!$isLeaveDate) {
                        $color = 'red'; // Default color for ACTIVE
                        if ($status === 'INACTIVE') {
                            $color = 'violet';
                        } elseif ($status === 'STANDBY') {
                            $color = 'black';
    ...
    public function getAttendanceData(Request $request, $coachId)
    {
        ...
        if ($type === 'Batch') {
            ...
            // Check if the batch status is ACTIVE
            if ($batch->status !== 'ACTIVE') {
                return response()->json(['error' => 'Batch is not ACTIVE'], 403);
            }

            // If the batch is active, handle the batch type
            return $this->handleBatchType($id, $coachId, $date);
        } elseif ($type === 'Demo') {
            return $this->handleDemoType($id, $coachId, $date);
        }
    }

    private function handleBatchType($id, $coachId, $date)
    {
        $data = Batch::where('id', $id)
            ->where('coach_id', $coachId)
            ->with(['batchSchedules', 'studentBatches.student', 'coach', 'parent'])
            ->first();
        if (!$data) {
        ...
        return view('Admin.CoachReports.batchattendance', [
            // batch timeline ::
    ...
    public function batchAttendance(Request $request, $coachId)
    {
        ...
        $rules = [
            ...
            'student_ids.*' => 'required|integer|exists:students,id', // Validate each student ID exists
        ...
        //Submit or update data within 10 minutes after the batch has ended ::
        $batchEndTime = Carbon::createFromFormat('Y-m-d h:i A', $request->input('date') . ' ' . $request->input('batchEndTiming'), 'Asia/Kolkata');
        $currentTime = Carbon::now('Asia/Kolkata');

        // Check if the current time is before the batch end time
        if ($currentTime->lt($batchEndTime)) {
            return response()->json(['error' => 'You cannot submit attendance before the batch has ended.'], 422);
        }
        // ---------------------------------
        ...
        // Coach Attendance ::
        // Check for any attendance record for this batch and coach
        $latestCoachAttendance = CoachAttendance::where('batch_id', $request->input('batch_id'))
            ->where('coach_id', $request->input('coach_id'))
            ->orderBy('id', 'desc')
            ->first();
        if ($latestCoachAttendance && \Carbon\Carbon::parse($latestCoachAttendance->date)->toDateString() == $attendanceDate) {
            // If attendance for the same day exists, update the existing record
            $previousStatus = $latestCoachAttendance->status;
            $latestCoachAttendance->status = $request->input('status');
            $coachAttendance = $latestCoachAttendance;
            // If updating from CANCELLED to COMPLETED, find the last non-cancelled session count
            if ($previousStatus == 'CANCELLED' && $request->input('status') == 'COMPLETED') {
                $lastCompletedSession = CoachAttendance::where('batch_id', $request->input('batch_id'))
                    ->where('coach_id', $request->input('coach_id'))
                    ->where('status', 'COMPLETED')
                    ->orderBy('id', 'desc')
                    ->first();
                $coachAttendance->number_of_batch_sessions = $lastCompletedSession ? $lastCompletedSession->number_of_batch_sessions + 1 : 1;
            } elseif ($previousStatus == 'COMPLETED' && $request->input('status') == 'CANCELLED') {
                // If updating from COMPLETED to CANCELLED, decrement the session count, ensuring it doesn't go below 1
                $coachAttendance->number_of_batch_sessions = max($latestCoachAttendance->number_of_batch_sessions - 1, 1);
            }
        } else {
            // Create a new record
            $coachAttendance = new CoachAttendance();
            $coachAttendance->coach_id = $request->input('coach_id');
            $coachAttendance->type = $request->input('type');
            $coachAttendance->batch_id = $request->input('batch_id');
            $coachAttendance->date = $request->input('date');
            $coachAttendance->time = $request->input('time');
            $coachAttendance->status = $request->input('status');
            if ($latestCoachAttendance && $request->input('status') != 'CANCELLED') {
                $coachAttendance->number_of_batch_sessions = $latestCoachAttendance->number_of_batch_sessions + 1;
            } else if ($latestCoachAttendance) {
                $coachAttendance->number_of_batch_sessions = $latestCoachAttendance->number_of_batch_sessions;
            } else {
                $coachAttendance->number_of_batch_sessions = 1;
            }
        }
        $coachAttendance->save();

        // Student Attendances ::
        // Assuming $request->status holds the overall operation status
        $overallStatus = $request->status;

        foreach ($request->student_ids as $studentId) {
            $latestStudentAttendance = StudentAttendance::where('batch_id', $request->input('batch_id'))
                ->where('student_id', $studentId)
                ->orderBy('id', 'desc')
                ->first();

            $studentStatus = $request->studentStatus[$studentId] ?? 'ABSENT';
            $studentRemark = $request->studentRemark[$studentId] ?? '';
            $attendanceDate = Carbon::parse($request->input('date'))->toDateString();

            if ($overallStatus == 'COMPLETED') {
                if ($latestStudentAttendance && Carbon::parse($latestStudentAttendance->date)->toDateString() == $attendanceDate) {
                    // Update the existing record
                    $latestStudentAttendance->status = $studentStatus;
                    $latestStudentAttendance->remark = $studentRemark;
                    $latestStudentAttendance->type = $request->input('type');
                    $latestStudentAttendance->coach_id = $request->input('coach_id');
                    $studentAttendance = $latestStudentAttendance;
                } else {
                    // Create a new record
                    $studentAttendance = new StudentAttendance();
                    $studentAttendance->student_id = $studentId;
                    $studentAttendance->batch_id = $request->input('batch_id');
                    $studentAttendance->date = $request->input('date');
                    $studentAttendance->time = $request->input('time');
                    $studentAttendance->status = $studentStatus;
                    $studentAttendance->remark = $studentRemark;
                    $studentAttendance->type = $request->input('type');
                    $studentAttendance->coach_id = $request->input('coach_id');
                    $studentAttendance->number_of_batch_sessions = $latestStudentAttendance ? $latestStudentAttendance->number_of_batch_sessions + 1 : 1;
                }
                // Save the record
                $studentAttendance->save();
            } elseif ($overallStatus == 'CANCELLED' && $latestStudentAttendance && Carbon::parse($latestStudentAttendance->date)->toDateString() == $attendanceDate) {
                // If the overall status is CANCELLED and there's an attendance record for the same day, delete it
                $latestStudentAttendance->delete();
            }
            // No else case needed, as only COMPLETED and CANCELLED are valid options per the validation rules
        }

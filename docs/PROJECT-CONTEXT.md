...
- Coach late popup is shown on coach dashboard home for today's unacknowledged `LATE` penalties, only once after coach acknowledgement; cancelled penalties do not show the popup and remain report-only.
```

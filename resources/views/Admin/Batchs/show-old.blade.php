                            ...
                            <div class="d-flex flex-column">
                                ...
                                <div class="d-flex align-items-center gap-2">
                                    @if ($totalSessionsCompleted > 0)
                                        ...
                                        <span class="fs-5 fw-semibold text-dark">Total Sessions Compleddted:
...
@php
    ...
    $activeStudentBatches = StudentBatch::whereHas('batch', function ($query) use ($batch) {
        ...
        ->with('batch', 'student', 'coach', 'level')
        ->get();
    //4

    // dd($activeStudentBatches);

    $studentBatchIds = $activeStudentBatches->pluck('batch_id')->unique();
    // dd($studentBatchIds);     0 => 4
    $batches = Batch::whereIn('id', $studentBatchIds)
        // ->where('id', '!=', $batch->id)
        ->with('studentBatches.student', 'studentBatches.coach', 'studentBatches.level')
        ->get();
    // dd($batches);
    // batch id 4 get
    ...
    $relatedBatches = Batch::whereIn('parent_id', $parentIds)
        ->with('studentBatches.student', 'studentBatches.coach', 'studentBatches.level')
        ->get();
    // dd($relatedBatches);

    $parentBatches = Batch::whereIn('id', $parentIds)
        ->with('studentBatches.student', 'studentBatches.coach', 'studentBatches.level')
        ->get()
        ->keyBy('id');
                ...
                <ul>
                    @foreach ($sortedChildBatches as $batch)
                        @if ($batch->id && $batch->status != 'ACTIVE')
                            @php
                                $uniqueStudentCounts = $batch->studentBatches->unique('student_id')->count();
                                $activeStudentCounts = $batch->studentBatches
                                    ->where('status', 'ACTIVE')
                                    ->unique('student_id')
                                    ->count();
                            @endphp
                                ...
                                <div class="col-12 col-md-12 mb-md-0">
                                    <div class="card shadow-sm rounded-lg p-3">
                                        <div class="d-flex align-items-center mb-2">
                                            <i class="ti ti-calendar fs-5 text-primary me-2"></i>
                                            <h5 class="fw-semibold mb-0">Old Batches Records</h5>
                                        </div>
                                        ...
                                        <tbody>
                                            @foreach ($batch->studentBatches as $studentBatch)
                                                <tr>
                                                    ...
                                                    <td>{{ $studentBatch->student->first_name }}
                                                        {{ $studentBatch->student->last_name }}
                                                        ({{ $studentBatch->student->student_id }})
                                                    </td>

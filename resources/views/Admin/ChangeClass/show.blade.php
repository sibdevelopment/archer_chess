    ...
    <div class="col-lg-4 order-lg-2 order-1">
        <div class="">
            ...
            <div class="text-center">
                <h5 class="fs-5 mb-0 fw-semibold">{{ $changeclass->student->first_name }}
                    {{ $changeclass->student->last_name }}</h5>
            </div>
        </div>
    </div>
    <div class="col-lg-8 order-last">
        <ul class="list-unstyled mb-0">
            @if ($changeclass->student->age)
                <li class="d-flex align-items-center gap-3 mb-4">
                    <i class="ti ti-stretching text-dark fs-6"></i>
                    <h6 class="fs-4 fw-semibold mb-0">Age : {{ $changeclass->student->age }}</h6>
                </li>
            @endif
            @if ($changeclass->student->mobile)
                <li class="d-flex align-items-center gap-3 mb-4">
                    <i class="ti ti-phone text-dark fs-6"></i>
                    <h6 class="fs-4 fw-semibold mb-0">Mobile : {{ $changeclass->student->mobile }}</h6>
                </li>
            @endif
            @if ($changeclass->student->user->device_id)
                <li class="d-flex align-items-center gap-3 mb-4">
                    <i class="ti ti-key text-dark fs-6"></i>
                    <h6 class="fs-4 fw-semibold mb-0">Password : {{ $changeclass->student->user->device_id }}</h6>
                </li>
            @endif
                <li class="d-flex align-items-center gap-3 mb-4">
                    <i class="ti ti-mail text-dark fs-6"></i>
                    <h6 class="fs-4 fw-semibold mb-0">Email : {{ $changeclass->student->user->email ?? $changeclass->student->email }}</h6>
                </li>

                <li class="d-flex align-items-center gap-3 mb-4">
                    <i class="ti ti-home text-dark fs-6"></i>
                    <h6 class="fs-4 fw-semibold mb-0">City : {{ $changeclass->student->city }}</h6>
                </li>

                <li class="d-flex align-items-center gap-3 mb-4">
                    <i class="ti ti-home text-dark fs-6"></i>
                    <h6 class="fs-4 fw-semibold mb-0">Country : {{ $changeclass->student->country }}</h6>
                </li>
            @if ($changeclass->student->student_id)
                <li class="d-flex align-items-center gap-3 mb-4">
                    <i class="ti ti-id-badge text-dark fs-6"></i>
                    <h6 class="fs-4 fw-semibold mb-0">Id : {{ $changeclass->student->student_id }}</h6>
                </li>
            @endif
        </ul>
    </div>
...
<form method="POST" action="{{ route('admin.changeclasses.update', $changeclass->id) }}"
                    ...
                    <div class="row">
                            ...
                            <div class="col-sm-12 col-md-6">
                                ...
                                <select class="form-control select2" name="employee_ids[]" multiple>
                                    ...
                                    <option value="{{ $employee->id }}"
                                        {{ in_array($employee->id, (array) $changeclass->employee_ids) ? 'selected' : '' }}>
                            ...
                            <select class="form-control select2" name="batch_id" id="" style="width: 100%;">
                                ...
                                @foreach ($batches as $batch)
                                    <option value="{{ $batch->id }}"
                                        {{ isset($changeclass->current_batch_id) && $changeclass->current_batch_id == $batch->id ? 'selected' : '' }}>
                        ...
                        <div class="col-sm-12 col-md-4">
                            ...
                            <input type="date" class="form-control" name="receive_date"
                                value="{{ $changeclass->receive_date ?? '' }}">
                        ...
                        <div class="col-sm-12 col-md-4">
                            ...
                            <input type="text" class="form-control" name="received_fees"
                                value="{{ $changeclass->received_fees ?? '' }}">
                            ...
                            <select class="form-control select2" name="payment_level_id">
                                ...
                                @foreach ($paymentlevels as $paymentlevel)
                                    <option value="{{ $paymentlevel->id }}"
                                        {{ old('payment_level_id', $changeclass->student->lastpayment_level_id ?? '') == $paymentlevel->id ? 'selected' : '' }}>
                        ...
                        <div class="col-sm-12 col-md-12">
                            ...
                            <textarea class="form-control" name="remark">{{ $changeclass->remark ?? '' }}</textarea>
                            <div id="remark-error" class="text-danger"></div>
                        </div>
                    </div>
                </div>
                @php
                    $student_fees = App\Models\StudentFee::where('student_id', $changeclass->student->id)->get();
                    // dd($student_fees);
                @endphp
                {{-- @if (count($student_fees) < 1) --}}
                @if($changeclass->is_submitted == 0)

                    <div class="card-footer">
                        <button type="button" class="btn btn-primary" id="changeclass">
...
<script>
    ...
    $(document).on("click", "#changeclass, #student-fee", function(e) {
        e.preventDefault();
        $('div[id$="-error"]').empty();
        ...
        var actionType = $(this).attr('id') === "changeclass" ? "changeclass" : "student-fee";

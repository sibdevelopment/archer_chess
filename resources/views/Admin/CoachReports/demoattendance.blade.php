...
@endif
<h6>Remember: You can only submit or update data within <span class="text-danger">15 minutes</span> after the demo ended.
</h6>
<!-- ------------------------------------------------------------------------------ :: -->
<!-- Table for displaying demolead names and additional information along with form elements for status and remark -->
<form method="POST" action="{{ route('admin.reports.demoAttendance', ['coachId' => $data->coach_id]) }}"
        ...
        <tbody>
            <tr>
                ...
                <td style="text-align: center;">
                    {{ date('h:i A', strtotime(explode(' - ', $data->slot)[0])) }} -
                    {{ date('h:i A', strtotime(explode(' - ', $data->slot)[1])) }}
                </td>
                <td style="text-align: center;">
                    <div class="col-sm-12 col-md-12">
                        <select class="form-control" name="status">
                            <option value="COMPLETED"
                                {{ $studentAttendances->first() && $studentAttendances->first()->status == 'COMPLETED' ? 'selected' : '' }}>
                                COMPLETED
                            </option>
                            <option value="CANCELLED"
                                {{ $studentAttendances->first() && $studentAttendances->first()->status == 'CANCELLED' ? 'selected' : '' }}>
                                CANCELLED
                            </option>
                        </select>
                        ...
                        <select class="form-control" name="level_id">
                            ...
                            @foreach ($levels as $level)
                                <option value="{{ $level->id }}"
                                    {{ $studentAttendances->first() && $studentAttendances->first()->level_id == $level->id ? 'selected' : '' }}>
                                    {{ $level->name }}</option>
                            @endforeach
                        </select>
                ...
                <td style="text-align: center;">
                    {{-- <input type="text" class="form-control" name="remark" placeholder="Enter remarks here" --}}
    ...
    <div class="row">
        ...
        <div class="col-2">
            <input type="date" class="form-control" name="date"
                value="{{ $coachAttendance ? \Carbon\Carbon::parse($coachAttendance->date)->format('Y-m-d') : \Carbon\Carbon::parse($date)->format('Y-m-d') }}"
                readonly />
        ...
        <div class="col-2">
            @php
                ...
                $endTime = date('H:i', strtotime(explode(' - ', $data->slot)[1]));
            @endphp
            <input type="time" class="form-control" name="time"
                value="{{ $coachAttendance ? \Carbon\Carbon::parse($coachAttendance->time)->format('H:i') : $endTime }}"
                readonly />

    ...
    <div class="container">
        <div class="table-wrapper">
            <table class="table table-bordered">
                ...
                <tbody>
                    @foreach ($student_batches as $key => $student_batch)
                    @php
                        $studentinfo = App\Models\Student::where('id', $student_batch->student_id)->first();
                        $batchinfo = App\Models\Batch::where('id', $student_batch->batch_id)->first();
                    @endphp
                    <tr>
                        <td>{{ $key + 1 }}</td>
                        <td>{{ $studentinfo->first_name }} {{ $studentinfo->last_name }}</td>
                        <td>{{ $student_batch->student_id }}</td>
                        <td>{{ $student_batch->coach_id }}</td>
                        <td>{{ $student_batch->batch_id }} - {{ $batchinfo->name }}</td>
                        <td>{{ $student_batch->duplicate_count }}</td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
